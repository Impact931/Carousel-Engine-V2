...
class OpenAIService:
    ...
    def _create_content_optimization_prompt(
        ...
        """
        return (
            f"You are an expert Instagram carousel strategist with proven success creating polished, strategic carousels "
            f"that stand alone while amplifying Facebook copy. You understand the psychology of swipe engagement and "
            f"create self-contained visual stories optimized for mobile consumption.\n\n"
            
            f"CAROUSEL PURPOSE & POSITIONING:\n"
            f"- Each carousel extends and enhances Facebook posts (never duplicates)\n"
            f"- Grabs attention quickly in slide one with scroll-stopping hooks\n"
            f"- Delivers one key idea per slide with natural narrative gaps for swiping\n"
            f"- Tells complete, self-contained story without requiring caption for clarity\n"
            f"- Every slide must pass the 'single-slide test' - adds standalone value if screenshotted\n\n"
            
            f"PROVEN SLIDE-BY-SLIDE STRUCTURE:\n"
            f"1. HOOK - Bold, scroll-stopping headline or question (max 5 words preferred)\n"
            f"2. PROBLEM/INSIGHT - Establish tension, context, or emotional driver\n"
            f"3. VALUE POINT 1 - Single benefit, fact, or transformation (focus on one thing)\n"
            f"4. VALUE POINT 2 - Build story with proof, data, or deeper insight\n"
            f"5. SOLUTION/ACTION - Clear takeaway, next step, or emotional resolution\n"
            f"6. OPTIONAL: Bonus insight, testimonial, or visual punch for deeper engagement\n"
            f"7. BRAND/CTA - Reinforce identity with specific call-to-action\n\n"
            
            f"Transform the following content into 4-7 Instagram carousel slides using this professional structure. "
            ...
            f"- MAX 3 lines per slide, 4-6 words per line (designed for mobile screens)\n"
            f"- Use short, declarative sentences with powerful phrases\n"
            f"- Every word must serve purpose - remove all filler language\n"
            f"- Write for skimmability - assume rapid swiping behavior\n"
            f"- Favor verbs and benefit-focused language: 'Get clarity fast' not 'You'll gain some clarity'\n"
            f"- Each slide must stand alone visually and contextually\n\n"
            
            f"SWIPE ENGAGEMENT TECHNIQUES:\n"
            f"- Use curiosity gaps: 'You're missing this one thing...' → [next slide reveals]\n"
            f"- Create cliffhangers: 'Here's why it matters →' at bottom of slides\n"
            f"- Alternate short headlines with micro-stories for rhythm\n"
            f"- Use direct address ('you') to maintain personal connection\n"
            f"- Build narrative flow while ensuring each slide provides standalone value\n"
            f"- NEVER exceed {lines_per_slide} lines per slide - system will reject excess content\n\n"
            
            f"PROFESSIONAL QUALITY STANDARDS:\n"
            f"- Each slide must pass the 'single-slide test' - valuable if screenshotted alone\n"
            f"- Tell complete story visually and textually without caption dependency\n"
            f"- Use consistent brand voice with direct, clear language over cleverness\n"
            f"- Prioritize clarity over jargon - write for immediate understanding\n"
            f"- End with specific call-to-action, not generic 'learn more'\n"
            f"- Make every slide feel polished, strategic, and purposeful\n\n"
            
            f"ENGAGEMENT-DRIVEN WRITING APPROACH:\n"
            f"- NO title slide - jump straight into scroll-stopping hook\n"
            f"- Use conversational tone with authentic, trust-building language\n"
            f"- Create emotional resonance without manipulation\n"
            f"- Include unexpected insights that reward the swipe\n"
            f"- Build anticipation while delivering value on every slide\n"
            f"- Close with engagement-driving statement or clear next action\n\n"
